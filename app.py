...
if dados_ok:
    ...
    # ==============================
        ...
        if col_escola is None:
            st.warning("Coluna de escola não encontrada no CSV.")
            ...
            df_escolas_ano = (
                df.dropna(subset=["Ano", col_escola])
            ...
            total_escolas = df[col_escola].dropna().nunique()
            st.metric("Total de escolas distintas (todos os anos)", total_escolas)
            ...
            fig = px.bar(
                ...
                y="Total_Escolas",
                text="Total_Escolas",
                title="Total de Escolas por Ano",
                labels={"Ano_str": "Ano", "Total_Escolas": "Total de Escolas"},
                color="Ano_str",
                color_discrete_map=cores_anos,
            )
            fig.update_traces(textposition="outside")
            ...
            if len(df_escolas_ano) > 2:
                fig2 = px.line(
                    ...
                    labels={"Ano": "Ano", "Total_Escolas": "Total de Escolas"},
                )
                fig2.update_layout(
                    plot_bgcolor="rgba(0,0,0,0)",
                    xaxis=dict(
            ...
            st.subheader("Tabela resumo")
            st.dataframe(df_escolas_ano[["Ano_str", "Total_Escolas"]].rename(columns={"Ano_str": "Ano"}), use_container_width=True)
